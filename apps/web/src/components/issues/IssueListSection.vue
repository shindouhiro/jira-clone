...
<template>
  <section id="issue-list-section" aria-label="Issue list" class="space-y-4">
    <div v-if="isFetching && issues.length === 0" id="issue-list-loading" class="flex flex-col items-center justify-center gap-4 py-20" role="status" aria-live="polite">
      <div class="i-tabler-loader-2 animate-spin text-5xl text-teal-500" />
    ...
    <div
      v-else
      id="issue-list-empty"
      class="border-2 border-gray-100 rounded-3xl border-dashed py-20 text-center dark:border-gray-800"
      role="status"
      aria-live="polite"
    >
      <div class="i-tabler-mood-empty mx-auto mb-4 text-5xl text-gray-300 dark:text-gray-700" />
      <h3 class="text-xl text-gray-400 font-bold dark:text-gray-500">
        {{ t('common.no_bugs') }}
      </h3>
